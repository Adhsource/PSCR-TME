...
// lights, scene, screen, pixels,
public: 
	PixelJob(int x, int y, Scene &scene, vector<Vec3D>& lights, Color * pixels, const Scene::screen_t& screen) : 
...
int main () {
	...
	for (int x =0 ; x < scene.getWidth() ; x++) {
		for (int  y = 0 ; y < scene.getHeight() ; y++) {
			...
			// 	// mettre a jour la couleur du pixel dans l'image finale.
			// 	pixel = finalcolor;
			pool.submit(new PixelJob(x,y,scene,lights,pixels, screen)); 
			}
